...
Thread.yield();
// 정적메서드, 여러 개의 cpu 가 있을 때 자원을 양보(포기)하게 권장함(request), 스케줄러가 ㄴㄴ 할 수도 있음.

```

## Synchronized

synchronized keyword를 설정하는 순간, 단 한 가지 Thread만이 특정한 예시에 존재하는 코드를 작동시킬 수 있었어요.

이에 반해 다른 Thread가 동기화된 코드를 실행시키려면 접근이 허용될 때, 즉 작동하고 있는 Thread가 코드의 실행을 완료할 때까지 기다려야 합니다.

다음 단계에서는 Thread와 관련해서 가장 재미있는 주제인 Executor Service에 대해 배워보도록 할게요.


HashTable에 대한 코드를 열어보도록 할게요

여기 보시다시피 HashTable은 자바1에서부터 가능하고,

이런 식으로 동기화에 접근하는 방법 또한 자바1에서부터 가능해 왔다는 것을 알 수 있습니다

여기 이쪽을 보시면, 대부분의 메소드가 동기화 키워드를 가지고 있다는 것을 확인할 수 있어요 보시다시피 synchronized가 지속해서 언급되고 있죠

그러므로 HashTable에서의 거의 모든 메소드는 동기화된 키워드, 즉 synchronized keyword를 가집니다

그렇다면 여러분들은 지금 왜 HashTable이 동기화되어야 하는 건지에 대해 의문을 가지고 있으리라 생각해요.

여기 데이터 자료의 조각이 있다고 해볼게요.

이 두 가지의 Thread 사이에서 공유되는 데이터 자료가 존재한다고 해볼게요.

즉, 이 두 Thread가 HashTable의 예시를 공유하는 것이죠.

여기에 synchronized를 적어주면, Task1과 Task2 둘 중 하나의 Thread만 동기화된 메소드 안의 코드를 실행할 수 있죠.

이게 무슨 뜻인지 설명해 보도록 할게요 만약 특정 시점에서 동기화된 메소드가 50개 있다고 해볼게요.

그리고 이 중 하나의 Thread만 이 동기화된 50개의 메소드 중 아무 메소드를 실행시킬 수 있는 것이죠.
50개의 동기화가 끝난 메소드가 있어요.
그리고 여기서 동기화된 코드가 1,000줄이 있고, 이 중 단 한개의 Thread만이 이 1000줄의 코드를 실행할 수 있죠.
만약 이미 하나의 Thread가 1,000줄의 코드 중 어떤 코드이든지 실행하고 있다면,
다른 Thread는 첫 번째 Thread의 동기화 코드에 대한 실행이 완료될 때까지 대기하다가 메소드를 실행하기 위한 접근 권한을 얻기 전에 나와주어야 합니다.
이게 synchronized keyword의 중요성이라고 할 수 있어요.
동기화에서 이해하고 넘어가야 하는 정말 중요한 것 중 하나는 동기화가 overhead가 많이 생긴다는 점입니다.
이유는 이 1,000줄의 코드가 단 한 개의 Thread에 대해서만 실행이 가능하므로 다른 코드를 실행해야 하는 Thread 들은 무조건 대기를 해야 하기 때문이죠.
그러므로 시스템의 작동에 영향이 생길 확률이 높습니다 자바 5나 8과 같은 최근 버전에서는 Thread를 더 안전성 있게 사용이 가능하도록 하는 대안들이 많이 있습니다.
동기화를 사용해 접근하는 방법에만 의존하지 않는다는 이야기이죠.
concurrent Collection에 대해 배울 때 Thread에 안전하게 접근하는 방법들에 대해 알아볼 겁니다
...
1. 스레드 실행을 통제할 수 있단 보장이 없다.
   wait, join 으로만 스레드를 제어했을때의 문제점은 request 는 권유이기 때문에 Thread의 실행에 대해서 그 어떤 통제도 불가능하다는 점이에요, 그렇죠?

2. 정확한 실행여부와 시점을 알기 어렵다.
   또한 어느 특정 시점에서 얼마나 많은 Thread가 실행되고 있는지 알 수 없습니다. 
   예를 들어, 특정 시점에 3개의 스레드만 실행하는 것을 보장하기엔 Thread의 start() 메서드만으로는 매우 어렵습니다.

3. 조건을 주기 어렵다.
   예를 들어 task1, task2 둘 중 하나만 완료되어도 후속작업을 실행하게끔 작업하는건 기본 작업으로는 불가능합니다.

4. 또한, Thread를 실행하는 과정은 매우 지루한 과정이에요.
  ...
  만약에 100개의 task를 실행하고 싶다면, 이와 같은 코드를 모두 작성한 후에 시작을 실행해 주시면 됩니다 다시, 이 모든 코드를 작성하고, 실행을 눌러주시면 됩니다.
  이 코드를 묶음의 형식으로 처리하는 방법은 없습니다. ㅠㅠ

5. 마지막으로, 가끔 Thread가 실행한 task의 결과를 반환(return)하고 싶은 경우가 생길 수 있습니다.
  우리가 지금까지 배운 이야기한 내용으로는 불가능한 내용입니다. 
  
여기서 Executor Service, 즉 실행기를 사용할 수 있어요. 

1. 다수의 스레드의 실행 상태를 알 수 있다. (moniter)
2. 다수의 스레드를 묶어서 한번에 실행할 수 있다.
3. a | b 가 완료되면 알려주는 논리를 실행할 수 있다.
4. 모든 스레드의 실행완료를 기다릴 수 있다.
5. 이게 한 문장으로 가능하다.